...
def get_red(slider_value):
    ...
    # Turn the slider value into an int and hex value. Strip leading characters so only two remain.
...
def update_color():
    """Update the current slider box based on the slider values. Display tuple and hex values of the current"""
...
def store_color():
    ...
    # Create new widgets for the new color
    ...
    # Update the dict stored colors_ to the next value if available
...
def save_colors():
    """Output the chosen colors to a text file."""
...
# Put labels, sliders, and buttons on the frame....Use ipadx with rgb buttons to define column width, then use sticky on the others
...
for i in range(6):
    ...
    # .cget() Returns the value of a specific option. Store the hex value of the tuple label and hex label
    stored_colors[stored_color.get()] = [new_color_tuple.cget('text'), new_color_hex.cget('text')]
