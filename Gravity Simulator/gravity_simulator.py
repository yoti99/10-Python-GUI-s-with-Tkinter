...
# Define functions
def move(event):
    """Drag the ball vertically on the canvas to set the position."""
...
def step(t):
    ...
    for i in range(1, 5):
        ...
        if main_canvas.coords(balls['ball_%d' % i])[3] + d <= 415:
            ...
            # Draw dash line at the bottom of the ball
    ...
    # Update the times for the given time interval
    update_height()
...
def run():
    """Run the entire sim until all balls have hit the ground or left the screen based on the y2 coords [3]"""
    # Balls may start on the ground or at the top of the screen so call step() at least once
    step(t_slider.get())

    # Run step() until ALL balls have hit the ground or left the screen based on the y2 coords [3]
    while 15 < main_canvas.coords(balls['ball_1'])[3] < 415 or main_canvas.coords(balls['ball_2'])[3] < 415 or main_canvas.coords(balls['ball_3'])[3] < 415 or main_canvas.coords(balls['ball_4'])[3] < 415:
        step(t_slider.get())

def graph():
    """Graph distance Vs time for 4 balls"""
    # Colors of the balls correspond to colors of the graph
