...
def animate(button):
    """Animate a given button by changing it's color"""
...
def press(value):
    ...
    # Add the player press to players sequence
    player_sequence.append(value)
    print(player_sequence)

    # If current round is over, check to see if the player entered the correct sequence of button presses
    if len(player_sequence) == len(game_sequence):
        check_round()

def check_round():
    """Determine if the player entered the correct sequence of presses for a round."""
    global player_sequence
    global game_sequence
    global score

    # The player is correct, change the label, update the score, wait, then, start the next round
    if player_sequence == game_sequence:
        change_label("Correct!")
        score += len(player_sequence) + int(1000/time)
        root.after(500, pick_sequence)
    # The player is incorrect, change label, update score, disable buttons, and reset for new game.
    else:
        change_label('Wrong!')
        score = 0
        disable()
        ...
        game_sequence = []
        # Wait two seconds then change the message
...
# Layout for info frame
start_button = tkinter.Button(info_frame, text='New Game', font=game_font1, bg=game_color, command=enable)
score_label = tkinter.Label(info_frame, text="Score: " + str(score), font=game_font1, bg=game_color)
