...
def get_morse():
    ...
    # Remove any letters of symbols not in our dicts keys
    for letter in text:
    ...
    # Turn each individual word in word into a list of letters
    for word in word_list:
        ...
        for letter in letters:
            ...
            morse_code += " "
        # Seperate individual words with |
        morse_code += "|"
...
def get_english():
    ...
    # Remove all letters or symbols not in our dict keys
    for letter in text:
        if letter not in morse_to_english.keys():
            text = text.replace(letter, '')

    # Break up each word based on | and put into list
    word_list = text.split("|")

    # Turn each word into a list of letters
    for word in word_list:
        letters = word.split(" ")
        # For each letter, get english representation and add it to thr string english
        for letter in letters:
...
def show_guide():
    """Show a mouse code guide in a second window"""
    ...
    # Create a second window relative to the root window
...
english_to_morse = {'a': '.-', 'b': '-...', 'c': '-.-.', 'd': '-..', 'e': '.', 'f': '..-.', 'g': '--.', 'h': '....', 'i': '..', 'j': '.---', 'k': '-.-', 'l': '.-..', 'm': '--', 'n': '-.', 'o': '---', 'p': '.--.', 'q': '--.-', 'r': '.-.', 's': '...', 't': '-', 'u': '..--', 'v': '...-', 'w': '.--', 'x': '-..-', 'y': '-.--', 'z': '--..', '1': '.----', '2': '..---', '3': '...--', '4': '....-', '5': '.....', '6': '-....', '7': '--...', '8': '---..', '9': '----.', '0': '-----', ' ': ' ', '|': '|', "": ""}
...
convert_button.grid(row=0, column=0, padx=10, ipadx=50)  # Covert ipadx defines column width
